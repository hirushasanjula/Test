    ...
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        try {
          ...
          const userWithMethods = await User.findOne({ email });
    console.log("Authorize: User found:", {
      email: userWithMethods.email,
      passwordHash: userWithMethods.password,
    });
    const isPasswordValid = await userWithMethods.comparePassword(credentials.password);
    console.log("Authorize: Password valid:", isPasswordValid, "Input password:", credentials.password);
    if (!isPasswordValid) {
